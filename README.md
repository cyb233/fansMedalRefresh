...
7. 如需定时运行，请选择：
   ...
   > 通常建议运行间隔为 24 小时，因为脚本设计单次运行至多 24 小时

   - **Windows**：任务计划程序
   - **Linux / macOS**：`cron` 定时任务等

8. 更新
   > 当项目有更新时，根据下载项目的方式，可根据以下方式更新项目
   - 直接下载：
     - 重新下载并直接覆盖项目文件
   - Git 克隆方式：
     - 执行`git pull`拉取更新，或下载并直接覆盖项目文件

# 配置说明

- 除 users 和 push 外，各配置项均有默认值，无需改动可以不进行配置
- 配置时可参考`config.example.json`，或直接复制并重命名为`config.json`后修改
...
**_⚠️ 为了您的账号安全，如需提交日志，请注意自行对日志隐私内容打码（如`access_token`/`Cookie`等）！且通常建议提交`DEBUG`或`TRACE`级别日志_**
...
| 字段           | 类型        | 必填                  | 默认值 | 说明                                       |
| -------------- | ----------- | --------------------- | ------ | ------------------------------------------ |
| `enabled`      | `bool`      |                       | `true` | 是否启用该用户                             |
| `cookie`       | `str`       | ✅(`api_type`==`web`) | 无     | 用户的 B 站登录 Cookie，用于接口验证       |
| `access_token` | `str`       | ✅(`api_type`==`app`) | 无     | 用户的 B 站登录 access_token，用于接口验证 |
| `api_type`     | `str`       |                       | `app`  | API 调用类型，可选："web" / "app"          |
| `white_uids`   | `List[int]` |                       | `[]`   | 白名单 UID，仅这些用户会被操作             |
| `black_uids`   | `List[int]` |                       | `[]`   | 黑名单 UID，这些用户会被跳过               |

**说明：**

- `api_type`==`web`时`cookie` 不能为空，或 `api_type`==`app`时`access_token` 不能为空，否则会触发配置错误
- **获取`access_token`**：你可以通过`logintool.py`获取`access_token`，执行以下命令并从`login_info.json`复制`access_token`的值
  ```bash
  pip install requests qrcode rich
  python logintool.py
  ```
- **获取`cookie`**：你可以通过在已登陆的 B 站主站页面的地址栏输入以下 js 来获取并自行复制 cookie，注意 Chrome 等浏览器可能因安全原因自动删除开头的`javascript:`
  ```
  javascript: (() => {prompt("当前页面的 Cookie：", document.cookie || '');})();
  ```
  > 也可通过书签管理器将以上代码添加到书签来快速使用，只需在相应页面点击此书签即可
...
| `emoji_list`    | `List[str]` | 默认提供 10 条表情   | 表情列表（随机发送）     |
...
- 可自由自定义弹幕内容（表情、祝福语等）
- 弹幕内容为纯文本，建议控制长度
- 发送逻辑为`danmaku_list`和`emoji_list`的随机顺序组合
...
| `use_markdown`  | `bool` | `false` | 是否启用 Markdown 格式消息，启用后会对常见 Markdown 字符进行转义，避免出错，请注意各渠道是否支持 Markdown 格式 |
